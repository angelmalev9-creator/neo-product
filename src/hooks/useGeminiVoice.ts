...
export const useGeminiVoice = ({
  ...
  function normalizeBgForSpeech(text: string): string {
    ...
    return (
      text
        // Цени с евро/лева — "15999.00 €" → "15999 евро", "31291.32 лв." → "31291 лева"
        .replace(/(\d[\d\s]*(?:[.,]\d+)?)\s*€/g, "$1 евро")
        .replace(/(\d[\d\s]*(?:[.,]\d+)?)\s*лв\.?/g, "$1 лева")
        .replace(/(\d[\d\s]*(?:[.,]\d+)?)\s*EUR/gi, "$1 евро")
        .replace(/(\d[\d\s]*(?:[.,]\d+)?)\s*BGN/gi, "$1 лева")
        ...
        // Скорости/места — "7 места" остава, "7-степенна" остава
        ...
        // Двигателни типове — "V8" → "V осем", "V6" → "V шест" (Gemini ще го прочете добре)
        .replace(/\bV8\b/g, "V осем")
        .replace(/\bV6\b/g, "V шест")
        .replace(/\bV12\b/g, "V дванадесет")
        .replace(/\bV10\b/g, "V десет")
        // AMG/GTI и подобни — остави ги — Gemini ги произнася буква по буква добре
        ...
        sendToGemini(
          normalizeBgForSpeech(
            `${todayCtx}\n${focusBlock}\n` +
              ...
              `КРАТКО: макс 1-2 изречения. Клиентът СЛУША, не чете. ` +
              `Без "ъмм/мхм/аха/знаеш ли/виж". Професионален, топъл тон.]\n` +
              cleanText,
          ),
        );
      }
    ...
    async (systemPrompt: string, companyName: string, sessionId?: string) => {
      ...
      try {
        ...
        const BG_VOICE_PREFIX =
          ...
          `- Максимум 1-2 изречения на ход. НИКОГА повече от 3.\n` +
          `- Всеки отговор = 1 ясна мисъл + 1 следваща стъпка. Толкова.\n` +
          `- Предлагай 2 прости опции когато е нужен избор. Не 3, не 4.\n` +
          `- Кажи ясно какво следва: "Ще Ви разкажа за цените" или "Нека видим кое е по-подходящо".\n` +
          ...
          `- НИКОГА не пиши "ъмм", "мхм", "аха", "хмм", "знаеш ли", "виж", "представи си" в текста си.\n` +
          `- НИКОГА не звучи като робот, скрипт или автоматичен отговор.\n` +
          `- Бъди кратък и професионален — клиентът СЛУША, не чете.\n\n` +
          ...
          `- Давай КРАТКИ отговори — максимум 1-2 изречения на ход. НИКОГА повече от 3.\n` +
          `- Задавай само ЕДИН въпрос наведнъж — изчакай отговора на клиента.\n` +
          `- НЕ изреждай повече от 2 варианта — предложи 2, после питай.\n` +
          `- Говори разговорно и естествено — без списъци, без формален тон.\n` +
          `- Ако нещо не знаеш — кажи "Нека проверя" или "Не съм сигурен, но...".\n` +
          `- Отговаряй на зададения въпрос ДИРЕКТНО, без предисловие.\n` +
          `- ЗАБРАНЕНО е да пишеш звукоподражания като "ъмм", "мхм", "аха", "хмм" — те НЕ са думи.\n` +
          `- ЗАБРАНЕНО е да ползваш "знаеш ли", "виж", "представи си", "Толкова се радвам".\n\n` +
