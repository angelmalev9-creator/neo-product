...
import { Crown, User, CreditCard, Globe, Loader2, Save, Sparkles, Shield, Zap, ArrowUpRight, LogOut } from 'lucide-react';
...
const SettingsPage = ({
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      toast({
        title: '✓ Запазено',
        description: 'Промените са приложени'
      });
    } catch {
      toast({
        title: 'Грешка',
    ...
    <div className="h-full flex flex-col p-4 lg:p-6 overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-3 shrink-0">
        ...
        <Button
          ...
          className="gap-2 h-8 text-xs border-red-500/20 text-red-500 hover:bg-red-500/10"
        >
              ...
              {subscribed && (
                <div className="space-y-3 relative">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="rounded-xl bg-background/40 border border-border/10 p-3 text-center">
                      <p className="text-xl font-black text-foreground">
                        {formatUsageMinutes(remainingMinutes)}
                      </p>
                  ...
                  {usagePercent > 80 && (
                    <div className="flex items-center gap-2 rounded-lg bg-[hsl(var(--neo-warning))]/10 border border-[hsl(var(--neo-warning))]/20 p-2">
                      <Zap className="w-3.5 h-3.5 text-[hsl(var(--neo-warning))]" />
                      <p className="text-[11px] text-[hsl(var(--neo-warning))]">
                        Минутите ви свършват.
                      </p>
                    </div>
                  )}
                </div>
